...
export default registerMPElement(`mp-drop-menu`, class extends Component {
  ...
  attachedCallback() {
    ...
    this.clickOutsideListener = e => {
      const path = window.ShadowDOMPolyfill || typeof e.composedPath !== `function` ? e.path : e.composedPath();
      const clickWithinPreventCloseElement = this.state.preventCloseSelectors.some(selector => path.some(el => el.matches && el.matches(selector)));
      if (this.isAttributeEnabled(`open`) && clickWasOutside(e, this) && !clickWithinPreventCloseElement) {
        this._closeMenu();
      }
    };
  }
