...
function propertyToSegFilter(property, {isTypecastable=true} = {}) {
...
function propertyFilterToSegFilter(propertyFilter, {isTypecastable=true}={}) {
  ...
  return extend(propertyToSegFilter(propertyFilter.property, {isTypecastable}), {filter});
}
...
export function retentionQueryToUrlParams(query) {
  ...
  urlParams.filters = query.propertyFilters.map(filter => propertyFilterToSegFilter(filter, {isTypecastable: false}));
  ...
  if (query.bornEvent) {
    ...
    urlParams.born_filters = query.bornPropertyFilters.map(
      filter => propertyFilterToSegFilter(filter, {isTypecastable: false})
    );
  }
